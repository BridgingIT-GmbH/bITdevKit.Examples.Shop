    ...
    IChaosExceptionMessageHandler
{
    ...
    public override async Task Handle(EchoMessage message, CancellationToken cancellationToken)
    {
        ...
        using (this.Logger.BeginScope(loggerState))
        {
            ...
            this.Logger.LogInformation($"{{LogKey}} >>>>> INVENTORY echo {message.Text} (name={{MessageName}}, id={{MessageId}}, handler={{}}) ", Constants.LogKey, message.GetType().PrettyName(), message.Id, this.GetType().FullName);
        }
    }
}